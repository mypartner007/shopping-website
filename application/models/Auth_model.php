...
class Auth_model extends CI_Model
{
	...
	public function is_current_session($session_token){
		$ci =& get_instance();
		$ci->load->database();
		$session = null;
		//rrally db3
		$mysqli = new mysqli($ci->db->hostname, $ci->db->username, $ci->db->password, $ci->db->database1);
		if (!mysqli_connect_errno()) {
			$query = "SELECT session_token FROM users_sessions WHERE session_token = '" . $session_token . "';";
			if($result = $mysqli->query($query)) {
				while ( ($row = $result->fetch_assoc())) {
					$session = $row['session_token'];
					break;
				}
			}
			if($session != null)
