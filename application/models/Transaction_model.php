...
class Transaction_model extends CI_Model
{
    ...
    public function filter_w_transactions()
    {
        $data = array(
            'q' => $this->input->get('q', true)
        );
        $data['q'] = trim($data['q']);
        if (!empty($data['q'])) {
            $this->db->where('wallet_transactions.user_email like', '%'.$data['q'].'%');
        }
        else {
            $this->db->where('wallet_transactions.user_email like ', '%');
        }
    }
    ...
    public function get_w_transactions_count()
    {
        $this->filter_w_transactions();
    ...
    public function get_paginated_w_transactions($per_page, $offset)
    {
        $this->filter_w_transactions();
        $this->db->order_by('wallet_transactions.paid_at', 'DESC');
        $this->db->limit($per_page, $offset);
        $query = $this->db->get('wallet_transactions');
        return $query->result();
    }
