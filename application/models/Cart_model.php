...
class Cart_model extends CI_Model
{
	...
	//set wallet balance
	public function set_wallet_balance($balance, $useremail, $paid_amount)
	{
		$ci =& get_instance();
		$ci->load->database();
		// Connect to the database
		$mysqli1 = new mysqli($ci->db->hostname, $ci->db->username, $ci->db->password, $ci->db->database);
		$mysqli2 = new mysqli($ci->db->hostname, $ci->db->username, $ci->db->password, $ci->db->database1);
		
		// Check for errors
		if (!mysqli_connect_errno()) {
			//rrally_store
			$query1 = "UPDATE users SET user_wallet_balance = '". $balance . "' WHERE email = '". $useremail . "';";			
			//rrally_db3
			$query2 = "UPDATE users SET user_wallet_balance = '". $balance . "' WHERE user_email = '". $useremail . "';";
			$current_datetime = date("Y-m-d H:i:s");
			$query3 = "INSERT INTO wallet_transactions (user_email, paid_amount, paid_at) VALUES ('".$useremail."', '".$paid_amount."', '".$current_datetime."');";
			$mysqli1->query($query1);
			$mysqli2->query($query2);
			$mysqli1->query($query3);
			return true;
		} else {
			return false;
		}
	...
	//migrate store's wallet to community's wallet
	public function migrate_wallet($link_url, $node_type, $user_email, $payment_amount)
	{
		$ci =& get_instance();
		$ci->load->database();
		//rrally_db3
		$mysqli1 = new mysqli($ci->db->hostname, $ci->db->username, $ci->db->password, $ci->db->database1);
		
		// Check for errors
		if (!mysqli_connect_errno()) {			
			$query1 = "SELECT user_id FROM users WHERE user_email = '".$user_email."'";
			$result = $mysqli1->query($query1);
			...
			//rrally_db3
			$current_datetime = date("Y-m-d H:i:s");
			$query2 = "INSERT INTO ads_users_wallet_transactions (user_id, node_type, amount, type, date, store_linkurl) VALUES ('".$userid."', '".$node_type."', '".$payment_amount."', 'out', '".$current_datetime."', '".$link_url."');";
			$mysqli1->query($query2);
			return true;
		} else {
			return false;
		}
	...
	//init store wallet balance
	public function init_store_wallet_balance($user)
	{
		$ci =& get_instance();
		$ci->load->database();
		// Connect to the database
		...
		if($user != null) {
			if (!mysqli_connect_errno()) {
				$query1 = "SELECT user_wallet_balance FROM users WHERE user_email = '".$user->email."'";
				$result = $mysqli2->query($query1);
				while ( ($row = $result->fetch_assoc())) {
					$balance = $row['user_wallet_balance'];
					break;
				}

				//rrally_store
				$query2 = "UPDATE users SET user_wallet_balance = '". $balance . "' WHERE email = '". $user->email . "';";
				$mysqli1->query($query2);
				// return true;
			} else {
				// return false;
