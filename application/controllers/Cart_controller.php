...
class Cart_controller extends Home_Core_Controller
{
	...
	//Payment with Wallet
	public function wallet_payment_post()
	{
		$wallet_balance = $this->input->post('wallet_balance', true);
		$payment_amount = $this->input->post('payment_amount', true);
		$amount1 = substr($payment_amount,0,-2);
		$total_amount = $amount1 . '.' . substr($payment_amount,-2);
		$currency = $this->input->post('currency', true);
		$useremail = $this->input->post('useremail', true);
		
		$float_wallet_balance = floatval($wallet_balance);
		$float_total_amount = floatval($total_amount);
		
		if ($float_total_amount > $float_wallet_balance) {
			$this->session->set_flashdata('error', 'Need more wallet balance');
			$data = array(
				'status' => 0,
				'redirect' => lang_base_url() . "cart/payment"
			);
			...
			$balance = $float_wallet_balance - $float_total_amount;
			$new_balance = sprintf("%.2f", $balance);
			if ( $this->cart_model->set_wallet_balance($new_balance, $useremail, sprintf("%.2f", $total_amount)) ){
				...
				if ($mds_payment_type == 'sale') {
					//execute sale payment
					$this->execute_sale_payment($data_transaction, 'json_encode', 'wallet');
				} elseif ($mds_payment_type == 'promote') {
					//execute promote payment
					$this->execute_promote_payment($data_transaction, 'json_encode', 'wallet');
				}
			} else {
	 ...
	 * Execute Sale Payment
	 */
	public function execute_sale_payment($data_transaction, $redirect_type = 'json_encode', $method = 'other')
	{
		//add order
		...
		if (!empty($order)) {
			...
			if ($redirect_type == 'json_encode') {
				...
				if ($method == 'wallet') {
					$this->cart_model->migrate_wallet($data['redirect'], 'Order', $this->auth_user->email, $data_transaction['payment_amount']);
				}
				echo json_encode($data);
			} else {
				//return direct
	 ...
	 * Execute Promote Payment
	 */
	public function execute_promote_payment($data_transaction, $redirect_type = 'json_encode', $method = 'other')
	{
		$promoted_plan = $this->session->userdata('modesy_selected_promoted_plan');
		if (!empty($promoted_plan)) {
			//execute payment
			...
			if ($redirect_type == 'json_encode') {
				...
				if ($method == 'wallet') {
					$this->cart_model->migrate_wallet($data['redirect'], 'Promote', $this->auth_user->email, $data_transaction['payment_amount']);
				}
				echo json_encode($data);
			} else {
				redirect($lang_base_url . "promote-payment-completed?method=gtw&product_id=" . $promoted_plan->product_id);
