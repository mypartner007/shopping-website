...
class Order_admin_controller extends Admin_Core_Controller
{
    ...
    {
        $data['title'] = trans("wallet_transactions");
        $data['form_action'] = admin_url() . "wallet-transactions";

        $pagination = $this->paginate(admin_url() . 'wallet-transactions', $this->transaction_model->get_w_transactions_count());
        $data['w_transactions'] = $this->transaction_model->get_paginated_w_transactions($pagination['per_page'], $pagination['offset']);
        $data['admin_settings'] = get_admin_settings();
        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/order/wallet_transactions', $data);
        $this->load->view('admin/includes/_footer');
    }
