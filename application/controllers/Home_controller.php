...
class Home_controller extends Home_Core_Controller
{
	...
	public function index()
	{
		...
		if($this->auth_model->is_logged_in()){
			...
			if($this->auth_model->is_current_session($user->session_token)){
				$this->cart_model->init_store_wallet_balance($user);
			} else {
				$this->auth_model->logout();
			}
		}
		// $this->auth_user = $this->auth_model->get_logged_user();
			get_method();
	...
	public function token_check($token_key)
	{
		// if(!$this->auth_model->is_logged_in()){
			if(!empty($token_key)){
				if ($user = $this->auth_model->get_user_by_token($token_key)) {
					// echo "<script type='text/javascript'>alert('$user->session_token');</script>";
					if($this->auth_model->is_current_session($user->session_token)){
						$this->auth_model->login_direct($user);
						// $this->auth_user = $user;
						$this->auth_user = $this->auth_model->get_logged_user();
						$this->cart_model->init_store_wallet_balance($this->auth_user);
					} else {
						$this->auth_model->logout();
					}
				}
			} else {
				// header('Location: https://rrally.com/');
				header('Location: http://localhost/');
			}
		// }
		...
		// if($this->auth_model->get_logged_user()){
			get_method();
			// echo "<script type='text/javascript'>alert($this->auth_model->get_logged_user());</script>";
			// $data['user'] = $this->auth_model->get_logged_user();
			$data['title'] = $this->settings->homepage_title;
			$data['description'] = $this->settings->site_description;
			$data['keywords'] = $this->settings->keywords;
